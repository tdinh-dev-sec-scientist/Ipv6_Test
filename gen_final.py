...
def load_and_split(
    ...
    * The supervised training set balances classes by capping malicious samples
    ...
    # --- Malicious: DISJOINT 60 / 20 / 20 train / val / test (no leakage, R4.1) ---
    X_mal_train, X_mal_temp = train_test_split(
        X_malicious_raw, test_size=0.40, random_state=seed
    )
    X_mal_val, X_mal_test = train_test_split(
        X_mal_temp, test_size=0.50, random_state=seed
    )
    ...
    # --- Supervised training set (malicious TRAIN partition only; disjoint from val/test) ---
    n_train_normal = len(X_train_sc)
    X_mal_train_sc = scaler.transform(X_mal_train)   # was X_malicious_raw[:n_train_normal] (leaked)
    X_sup_train    = np.vstack([X_train_sc, X_mal_train_sc])
    y_sup_train    = np.array(
        [0]*n_train_normal + [1]*len(X_mal_train_sc), dtype=int
    )

    # --- Autoencoder inputs on a bounded [0,1] scale (R4.2 fix) ---
    # RobustScaler (above) is fine for the tree/kernel/boundary models, but it
    # leaves reconstruction error on an unbounded scale (the ~80k / 7.26e6 values
    # the reviewer flagged). The AE gets its OWN MinMax scaler, fit on
    # training-normal only, so per-sample MSE lies in [0,1], consistent with the
    # averaged-MSE loss (Eq. 4). Row order (normal then malicious) matches
    # y_val_all / y_test_all.
    ae_scaler  = MinMaxScaler().fit(X_norm_train)
    X_ae_train = ae_scaler.transform(X_norm_train)
    X_ae_val   = np.vstack([ae_scaler.transform(X_norm_val),
                            ae_scaler.transform(X_mal_val)])
    X_ae_test  = np.vstack([ae_scaler.transform(X_norm_test),
                            ae_scaler.transform(X_mal_test)])

    split_info = {
        "train_normal" : len(X_train_sc),
        "train_malicious": len(X_mal_train),
...
def train_all_models(data: Dict, cfg: Dict) -> List[Dict]:
    ...
    X_ae_train  = data["X_ae_train"]
    X_ae_val    = data["X_ae_val"]
    X_ae_test   = data["X_ae_test"]
    ...
    ae_val_sc  = np.mean((X_ae_val  - ae.predict(X_ae_val,  verbose=0))**2, axis=1)
    ae_test_sc = np.mean((X_ae_test - ae.predict(X_ae_test, verbose=0))**2, axis=1)
